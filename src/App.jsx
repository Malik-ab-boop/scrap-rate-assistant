  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', Tahoma, sans-serif", background: "#f0f4f8", minHeight: "100vh", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        {tab === "dashboard" && <Dashboard rates={rates} />}
        {tab === "calc" && <Calculator rates={rates} onSave={addTransaction} />}
        {tab === "history" && <History transactions={transactions} onDelete={deleteTransaction} />}
        {tab === "admin" && <Admin rates={rates} setRates={setRates} />}
      </div>
    ...
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
        {ITEMS.map((item) => (
          <div key={item.id} onClick={() => setLookup(lookup === item.id ? null : item.id)}
            ...
            <div style={{ fontSize: 22, fontWeight: 700, color: "#1a3a2a" }}>{fmt(rates[item.id])}</div>
      ...
      {lookup && (
        <div style={{ background: "#1a3a2a", color: "#fff", borderRadius: 16, padding: "18px 20px", marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 14, color: "#a7f3d0", marginTop: 2 }}>per kg of {ITEMS.find(i => i.id === lookup)?.name}</div>
        </div>
      )}
...
function Calculator({ rates, onSave }) {
  ...
  function handleSave() {
    ...
    const items = validRows.map((r, i) => ({
      itemId: r.itemId,
      itemName: ITEMS.find(it => it.id === r.itemId)?.name,
        ...
        <div style={{ background: "#1a3a2a", color: "#fff", borderRadius: 16, padding: "20px", marginBottom: 14 }}>
          ...
          <div style={{ marginTop: 14, borderTop: "1px solid #2d5a3d", paddingTop: 12 }}>
            {rows.map((r, i) => {
              ...
              const itm = ITEMS.find(it => it.id === r.itemId);
              return (
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {filtered.length === 0 ? (
        ...
        filtered.map((tx) => {
          // Support both new multi-item and old single-item format
  ...
  return (
    <div style={{ padding: 16 }}>
        ...
        <div key={itm.id} style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 12, padding: "14px 16px", marginBottom: 10, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input type="number" value={draft[itm.id]} onChange={e => setDraft(d => ({ ...d, [itm.id]: Number(e.target.value) }))}
      ...
      <button onClick={handleSave}
        style={{ width: "100%", marginTop: 8, padding: "16px", fontSize: 16, fontWeight: 700, background: saved ? "#15803d" : "#2d7a4f", color: "#fff", border: "none", borderRadius: 14, cursor: "pointer" }}>
